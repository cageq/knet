...
public:
	HttpResponse(const std::string& rsp = "", uint32_t c = 200) {
	    ...
	    http_encoder = std::make_shared<HttpEncoder<HttpResponse>>(*this);
	...
	inline uint32_t parse_response(const char* data, uint32_t len, bool inplace = false) {
		if (!http_decoder)
		{
			http_decoder =  std::make_shared<HttpDecoder<HttpResponse>>(*this);
		}
	...
	std::shared_ptr<HttpDecoder<HttpResponse>> http_decoder;
	std::shared_ptr<HttpEncoder<HttpResponse>> http_encoder;
};
